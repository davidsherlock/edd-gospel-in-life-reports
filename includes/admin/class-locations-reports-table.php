...
/**
 * Earnings by Tag Reports Table Class
 *
 * @package     EDD
 ...
 * @since 2.4
 */
class EDD_Locations_Reports_Table extends WP_List_Table {
	...
	public function get_columns() {
		$columns = array(
			'label'          => __( 'Location', 'edd-gospel-in-life-reports' ),
			'total_sales'    => __( 'Total Sales', 'edd-gospel-in-life-reports' ),
			'total_earnings' => __( 'Total Earnings', 'edd-gospel-in-life-reports' ),
			'avg_sales'      => __( 'Monthly Sales Avg', 'edd-gospel-in-life-reports' ),
			'avg_earnings'   => __( 'Monthly Earnings Avg', 'edd-gospel-in-life-reports' ),
		);
	...
	public function reports_data() {
		...
		if ( !empty( $dates[ 'year' ] ) ) {
			...
			$cached_report_key = 'edd_earnings_by_location_data' . $start_date . '_' . $end_date;
		} else {
			$start_date        = false;
			$end_date          = false;
			$cached_report_key = 'edd_earnings_by_location_data';
		}
		...
		if ( false !== $cached_reports ) {
			...
			$categories = get_terms( 'download_location', $term_args );
			...
			foreach ( $categories as $category_id => $category ) {
				...
				$child_terms = get_terms( 'download_location', $child_args );
				...
				if ( ! empty( $avg_earnings ) && $avg_sales < 1 ) {
					$avg_sales = __( 'Less than 1', 'edd-gospel-in-life-reports' );
				} else {
	...
	public function output_sales_graph() {
		...
		if ( empty( $total_sales ) ) {
			echo '<p><em>' . __( 'No sales for dates provided.', 'edd-gospel-in-life-reports' ) . '</em></p>';
		}

		// Sort High to Low, prior to filter so people can reorder if they please
		arsort( $data );
		$data = apply_filters( 'edd_location_sales_graph_data', $data );

		$options = apply_filters( 'edd_location_sales_graph_options', array(
	...
	public function output_earnings_graph() {
		...
		if ( empty( $total_earnings ) ) {
			echo '<p><em>' . __( 'No earnings for dates provided.', 'edd-gospel-in-life-reports' ) . '</em></p>';
		}

		// Sort High to Low, prior to filter so people can reorder if they please
		arsort( $data );
		$data = apply_filters( 'edd_location_earnings_graph_data', $data );

		$options = apply_filters( 'edd_location_earnings_graph_options', array(
