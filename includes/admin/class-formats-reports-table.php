 ...
 * @since 2.4
 */
class EDD_Formats_Reports_Table extends WP_List_Table {
	...
	public function get_columns() {
		$columns = array(
			'label'          => __( 'Format', 'edd-gospel-in-life-reports' ),
	...
	public function reports_data() {
		...
		if ( !empty( $dates[ 'year' ] ) ) {
			...
			$cached_report_key = 'edd_earnings_by_format_data' . $start_date . '_' . $end_date;
		} else {
			$start_date        = false;
			$end_date          = false;
			$cached_report_key = 'edd_earnings_by_format_data';
		}
		...
		if ( false !== $cached_reports ) {
			...
			$categories = get_terms( 'download_format', $term_args );
			...
			foreach ( $categories as $category_id => $category ) {
				...
				$child_terms = get_terms( 'download_format', $child_args );
	...
	public function output_sales_graph() {
		...
		arsort( $data );
		$data = apply_filters( 'edd_format_sales_graph_data', $data );

		$options = apply_filters( 'edd_format_sales_graph_options', array(
	...
	public function output_earnings_graph() {
		...
		// Sort High to Low, prior to filter so people can reorder if they please
		arsort( $data );
		$data = apply_filters( 'edd_format_earnings_graph_data', $data );

		$options = apply_filters( 'edd_format_earnings_graph_options', array(
